...
export default function ModernChatInterface({ invitation, onBack }: ModernChatInterfaceProps) {
  const { user } = useAuth();
  ...
  const otherUser = invitation.fromUser.id === user?.id ? invitation.toUser : invitation.fromUser;

  // Join the invitation room when component mounts
  useEffect(() => {
    if (invitation.id) {
      joinInvitationRoom(invitation.id.toString());
    }
  }, [invitation.id, joinInvitationRoom]);

  // Get chat messages
  const { data: chats = [], isLoading } = useQuery({
    queryKey: ["/api/invitations", invitation.id, "chats"],
    queryFn: async () => {
      const response = await apiRequest("GET", `/api/invitations/${invitation.id}/chats`);
      ...
      <header className="bg-white/90 backdrop-blur-md shadow-sm border-b px-4 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <span>{getWorkoutEmoji(invitation.workoutType || "")} {invitation.workoutType}</span>
              <span>•</span>
        ...
        <div className="flex items-center gap-2">
          <Badge variant="secondary" className="bg-green-100 text-green-800">
            {invitation.status === 'accepted' ? 'Geaccepteerd' : invitation.status}
          </Badge>
